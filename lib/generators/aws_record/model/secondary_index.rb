    ...
    class SecondaryIndex
      ...
      def initialize(name, opts)
        ...
        if opts.key? :projection_type
          raise ArgumentError, "Invalid projection type #{opts[:projection_type]}" unless PROJ_TYPES.include? opts[:projection_type]
          raise NotImplementedError, 'ALL is the only projection type currently supported' if opts[:projection_type] != 'ALL'
        else
          opts[:projection_type] = 'ALL'
        end

        raise ArgumentError, "#{opts[:hash_key]} cannot be both the rkey and hkey for gsi #{name}" if opts[:hash_key] == opts[:range_key]
